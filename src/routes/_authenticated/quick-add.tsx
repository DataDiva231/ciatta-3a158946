...
function QuickAddPage() {
  ...
  const save = () => {
    ...
    setSaveFailed(!ok);
    // Only remember it as "saved" once it actually persisted — otherwise a
    // retry tap would hit the `if (saved) return saved` guard above and
    // never try writing again.
    if (ok) setSaved(event);
    return event;
  };
  ...
  // "Understanding updated" is a moment, not a screen: it returns to Today on
  // its own — but only once the save actually landed. A failed write stays
  // on this screen so the person can retry, instead of being carried past a
  // "Thank you." for something that never happened.
  useEffect(() => {
    if (!done || saveFailed) return;
    try {
    ...
    if (saveFailed) {
      return (
        <div className="flex min-h-full flex-col justify-center px-7 pb-12">
          <h1 className="animate-in fade-in text-center font-serif text-[28px] leading-[1.15] tracking-[-0.015em] duration-300">
            That didn&apos;t save.
          </h1>
          <p className="animate-in fade-in mt-3 text-center text-[13.5px] leading-relaxed text-muted-foreground duration-500">
            Try once more — nothing here is lost, it just hasn&apos;t been written down yet.
          </p>
          <button
            type="button"
            onClick={() => save()}
            className="mx-auto mt-8 w-full max-w-[19rem] rounded-full bg-foreground px-6 py-[15px] text-[15px] font-medium text-background transition-all duration-200 active:scale-[0.99]"
          >
            Try again
          </button>
        </div>
